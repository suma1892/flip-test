...
export default function transactionListsPage() {
    ...
    const getData=()=>{
      ...
      dispatch(transactionListActions());
    }
    ...
    async function setSearchText(event) {
        ...
        const dataClone = await dataList.filter(row => {
          return (row[0].beneficiary_name.toLowerCase().includes(searchText.toLowerCase()) || row[0].sender_bank.toLowerCase().includes(searchText.toLowerCase())) || row[0].beneficiary_bank.toLowerCase().includes(searchText.toLowerCase()) || JSON.stringify(row[0].amount).toLowerCase().includes(searchText.toLowerCase())
        });
        setData(dataClone);
    }
    ...
    const Item = ({ item, props }) => (
      <TouchableOpacity 
      onPress={()=> navigation.navigate('detail',{data: item[0]})}
      style={[s.View2]} >
